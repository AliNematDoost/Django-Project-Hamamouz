...
class NamespaceListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if not cluster_id or not name:
            return Response({"error": "cluster_id and name are required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if Namespace.objects.filter(cluster=cluster, name=name).exists():
            return Response({"error": "Namespace already recorded for this cluster"}, status=status.HTTP_409_CONFLICT)
        ...
        try:
            ...
            if e.status == 409:
                ...
                return Response({"error": "Not authorized to create namespace"}, status=e.status)
            else:
