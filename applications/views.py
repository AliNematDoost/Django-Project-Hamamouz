...
class AppListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if not namespace_id or not name or not image:
            return Response(
                {"error": "namespace_id, name and image are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if not str(namespace_id).isdigit():
            return Response(
        ...
        if App.objects.filter(namespace=namespace, name=name).exists():
            return Response(
                {"error": "App already recorded for this namespace"},
        ...
        try:
            ...
            if e.status == 409:
                return Response(
                    {"error": "Deployment already exists in Kubernetes"},
                    status=status.HTTP_409_CONFLICT,
                )

            elif e.status in (401, 403):
                return Response(
                    {"error": "Not authorized to create deployment"}, status=e.status
                )
        ...
        try:
            ...
            return Response(
                {
                    "error": "Deployment created in Kubernetes but failed to save DB record",
                    "detail": str(e),
                },
...
class AppDetailView(APIView):
    def patch(self, request, pk):
        try:
            app = App.objects.select_related("namespace", "namespace__cluster").get(
                id=pk
            )
        ...
        try:
            ...
            if e.status == 404:
                ...
                return Response(
                    {"error": "Not authorized to update deployment"}, status=e.status
                )
    ...
    def delete(self, request, pk):
        try:
            with transaction.atomic():
                ...
                app = (
                    App.objects.select_related("namespace", "namespace__cluster")
                    .select_for_update()
                    .get(id=pk)
                )
